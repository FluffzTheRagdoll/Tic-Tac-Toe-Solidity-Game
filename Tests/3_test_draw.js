...
contract("ThreeInARow Test Draw", accounts => {
    it("should be possible to end the game without a winner", async () => {
        ...
        let txReceipt = await gameManagerInstance.startNewGame({from: accounts[0], value: web3.utils.toWei("1","ether")});
        let threeInARowInstance = await ThreeInARow.at(txReceipt.logs[0].args._gameAddress);
        let txReceiptJoin = await threeInARowInstance.joinGame({from: accounts[1],value: web3.utils.toWei("1", "ether")});
        ...
        let txReceiptPlayed = await threeInARowInstance.setStone(0,0,{from: txReceiptPlayed.logs[1].args._player});
        ...
        if(txReceiptPlayed.logs[0].args._reciever == accounts[0]) {
            ...
            assert.equal(web3.utils.toWei("1","ether"), txReceiptPlayed.logs[0].args._amountInWei, "Payout must be 1 Ether to the first account");
            assert.equal(accounts[1], txReceiptPlayed.logs[1].args._reciever, "Payout must be the second account");
            assert.equal(web3.utils.toWei("1","ether"), txReceiptPlayed.logs[1].args._amountInWei, "Payout must be 1 Ether to the second account");
        } else {
            assert.equal(accounts[1], txReceiptPlayed.logs[0].args._reciever, "Payout must be the first account");
            assert.equal(web3.utils.toWei("1","ether"), txReceiptPlayed.logs[1].args._amountInWei, "Payout must be 1 Ether to the first account");
            assert.equal(accounts[0], txReceiptPlayed.logs[1].args._reciever, "Payout must be the second account");
            assert.equal(web3.utils.toWei("1","ether"), txReceiptPlayed.logs[0].args._amountInWei, "Payout must be 1 Ether to the second account");
        }
        ...
        //assert.equal(balancePlayer1AfterDraw, parseInt(balancePlayer1BeforeDraw) + parseInt(web3.utils.toWei("1","ether")),"There should be a payback of 1 Ether");
        //assert.equal(balancePlayer2AfterDraw, parseInt(balancePlayer2BeforeDraw) + parseInt(web3.utils.toWei("1","ether")),"There should be a payback of 1 Ether");
