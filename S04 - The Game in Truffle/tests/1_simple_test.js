...
contract("ThreeInARow Test", accounts => {
    it("should have an empty game board in the beginning", async () => {
        let gameManagerInstance = await GameManager.deployed();
        let txReceipt = await gameManagerInstance.startNewGame({value: web3.utils.toWei("1","ether")});
        ...
        for(let i = 0; i < 3; i++) {
            for (let j = 0; j < 3; j++) {
                assert.equal(1, board[i][j], i + "Row, " + j + "Column must be zero");
            }
        }
    })
});